		    ...
		    BITSET64 c, BITSET64 u, int noPlies, int o, int minscore, int maxscore)
{
  ...
  int pid = fork (); //fork process and record process ID so that the parents and child can be identified
  if (pid == 0) //if the process is the child process
    {
      ...
      for(int i = 0; i < noOfMoves; i++)//for i in noOfMoves do
      {
        ...
        {
          int abresult = alphaBeta (l[i], c, u, noPlies, o, minscore, maxscore); //alphabeta searches move i and abresult is sent to the mailbox
          mailbox_send(mailbox_init, abresult, i, positionsExplored);//pass move_score, i, positions_explored back via mailbox using mailbox_send
        }
      }
  ...
  else //if the process is the parent process
    {
      ...
      for (i=0; i < noOfMoves; i++) //run as many times as there are moves
        {
          printf ("parent waiting for a result\n"); //notify user that the programs waiting for a result 
                                                    //since it can take a while if moves are still being searched
          mailbox_rec (barrier, &move_score, &move_index, &positions_explored); //recieve variables passed by reference
